...
namespace Vapps.Tests.MultiTenancy
{
    public class TenantAppService_Tests : AppTestBase
    {
        ...
        public async Task Create_Update_And_Delete_Tenant_Test()
        {
            ...
            //Assert
            editDto.Tenant.TenancyName.ShouldBe("testTenant");
            editDto.Tenant.Name.ShouldBe("Tenant for test purpose");
            editDto.Tenant.IsActive.ShouldBe(true);

            // UPDATE ----------------------------------
            editDto.Tenant.Name = "edited tenant name";
            editDto.Tenant.IsActive = false;
            await _tenantAppService.UpdateTenant(editDto.Tenant);
