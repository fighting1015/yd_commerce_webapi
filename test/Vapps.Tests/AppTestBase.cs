...
namespace Vapps.Tests
{
    ...
    /// Provides methods to easily work with <see cref="VappsDbContext"/>.
    /// </summary>
    public abstract class AppTestBase : AbpIntegratedTestBase<VappsTestModule>
    {
        ...
        {
            void NormalizeDbContext(VappsDbContext context)
            {
        ...
        protected void UsingDbContext(Action<VappsDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<VappsDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<VappsDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<VappsDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<VappsDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<VappsDbContext>())
                {
                    action(context);
                    context.SaveChanges();
                }
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<VappsDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<VappsDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<VappsDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<VappsDbContext, Task<T>> func)
        {
            T result;
