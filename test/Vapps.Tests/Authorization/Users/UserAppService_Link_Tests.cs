...
namespace Vapps.Tests.Authorization.Users
{
    public class UserAppService_Link_Tests : UserAppServiceTestBase
    {
        ...
        private async Task<int> CreateTestTenantAndTestUser()
        {
            ...
            {
                ConnectionString = SimpleStringCipher.Instance.Encrypt("Server=192.168.1.23; Database=VappsTest_" + Guid.NewGuid().ToString("N") + "; Trusted_Connection=True;")
            };
            ...
            using (var uow = _unitOfWorkManager.Begin())
            {
                using (_unitOfWorkManager.Current.SetTenantId(testTenant.Id))
                {
                    ...
                    UsingDbContext(null, (Action<EntityFrameworkCore.VappsDbContext>)(context =>
                    {
                        context.UserAccounts.Add(new Account
                        {
