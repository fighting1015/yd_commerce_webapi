...
namespace Vapps.Tests.Authorization.Users
{
    ...
    {
        public UserAppService_Update_Tests()
        {
        ...
        [Fact]
        public async Task Should_Not_Update_User_With_Duplicate_Username_Or_EmailAddress_Or_PhoneNumber()
        {
            //Arrange

            CreateTestUsers();
            var jnashUser = await GetUserByUserNameOrNullAsync("jnash");
            var xiaoyuyueUser = await GetUserByUserNameOrNullAsync("vapps");
            //Act
            ...
            //Try tp update with existing phone number
            exception = await Assert.ThrowsAsync<UserFriendlyException>(async () =>
             await UserAppService.CreateOrUpdateUser(
                 new CreateOrUpdateUserInput
                 {
                     ...
                     {
                         Id = jnashUser.Id,
                         EmailAddress = "694978795@gmail.com", //Changed email to an existing user
                         Name = "John",
                         Surname = "Nash",
                         UserName = "jnash",
                         Password = "123qwe",
                         PhoneNumber = "13750013054"
                     },
                     AssignedRoleNames = new string[0]
                 }));

            exception.Message.ShouldContain("13750013054");
        }
