...
namespace Vapps.Url
{
    public abstract class AppUrlServiceBase : IAppUrlService, ITransientDependency
    {
        ...
        {
            var activationLink = WebUrlService.GetBusinessCenterAddress(tenancyName).EnsureEndsWith('/') + EmailActivationRoute + "?userId={userId}&confirmationCode={confirmationCode}";
        ...
        {
            var resetLink = WebUrlService.GetBusinessCenterAddress(tenancyName).EnsureEndsWith('/') + PasswordResetRoute + "?userId={userId}&resetCode={resetCode}";
