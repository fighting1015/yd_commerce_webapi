...
{
    public class UserLinkManager : VappsDomainServiceBase, IUserLinkManager
    {
        ...
        /// 关联账号
        /// </summary>
        /// <param name="firstUser">当前账号</param>
        /// <param name="secondUser">目标账号</param>
        /// <returns></returns>
        [UnitOfWork]
        public virtual async Task Link(User firstUser, User secondUser)
        {
            ...
            var userAccountsToLink = secondUserAccount.UserLinkId.HasValue
                ? _userAccountRepository.GetAllList((System.Linq.Expressions.Expression<Func<Account, bool>>)(ua => ua.UserLinkId == secondUserAccount.UserLinkId.Value))
                : new List<Account> { secondUserAccount };
        ...
        [UnitOfWork]
        public virtual async Task<Account> GetUserAccountAsync(UserIdentifier userIdentifier)
        {
