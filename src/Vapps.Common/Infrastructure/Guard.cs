...
namespace Vapps.Infrastructure
{
    public class Guard
    {
        ...
        /// <exception cref="InvalidOperationException">If class reference is null.</exception>
        [DebuggerStepThrough]
        public static void NotNull<TValue, Content>(Func<TValue> value, Content content)
        {
            if (value() == null)
                throw new InvalidOperationException("'{0}' cannot be null.".FormatInvariant(value));

            if (content == null)
                throw new InvalidOperationException("'{0}' cannot be null.".FormatInvariant(content));
        }
        ...
        {
            return expression.GetMethodInfo().Name;
        }
