...
{
    public class IncomeStatisticsService : VappsAppServiceBase, IIncomeStatisticsService
    {
        ...
        private async Task<List<IncomeStastistic>> GetDailyIncomeStatisticsData(DateTime startDate, DateTime endDate)
        {
            var dailyRecords = await _subscriptionPaymentRepository.GetAll()
                .Where(s => s.CreationTime >= startDate &&
                            ...
                            .ToListAsync();
            var dailyRecordss = dailyRecords.GroupBy(s => new DateTime(s.CreationTime.Year, s.CreationTime.Month, s.CreationTime.Day));

            var dailyRecordsss = dailyRecordss.Select(s => new IncomeStastistic
            {
                Date = s.First().CreationTime.Date,
                Amount = s.DefaultIfEmpty(new SubscriptionPayment()).Sum(c => c.Amount)
            }).ToList();

            FillGapsInDailyIncomeStatistics(dailyRecordsss, startDate, endDate);
            return dailyRecordsss.OrderBy(s => s.Date).ToList();
        }
